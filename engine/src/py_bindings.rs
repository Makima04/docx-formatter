...
fn assemble_docx(doc_json: String, template_json: String, output_path: String) -> PyResult<String> {
    ...
    assembler::assemble_docx(&doc, &template, &output_path, None)
        .map_err(|e| pyo3::exceptions::PyRuntimeError::new_err(e))?;
...
fn assemble_docx_with_plan(
    ...
    let template: TemplateConfig = serde_json::from_str(&template_json)
        .map_err(|e| pyo3::exceptions::PyValueError::new_err(format!("Invalid template JSON: {e}")))?;
    let plan: LayoutPlan = serde_json::from_str(&plan_json)
        .map_err(|e| pyo3::exceptions::PyValueError::new_err(format!("Invalid plan JSON: {e}")))?;

    assembler::assemble_docx(&doc, &template, &output_path, Some(&plan))
        .map_err(|e| pyo3::exceptions::PyRuntimeError::new_err(e))?;
...
fn docx_fmt_core(m: &Bound<'_, PyModule>) -> PyResult<()> {
    ...
    m.add_function(wrap_pyfunction!(assemble_docx_with_plan, m)?)?;
